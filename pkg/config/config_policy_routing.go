...
import (
	...
	"github.com/GoogleCloudPlatform/netd/pkg/utils"
)
...
func init() {
	...
	if err := fillLocalRulesFromNode(clientset, nodeName); err != nil {
		glog.Errorf("failed to configure local rule destinations from node: %v", err)
		return
	}
...
func fillLocalRulesFromNode(clientset kubernetes.Interface, nodeName string) error {
	...
	podCIDRs, err := utils.GetPodCIDRs(node)
	if err != nil {
		return err
	}
