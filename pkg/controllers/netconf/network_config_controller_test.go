...
func TestNewNetworkConfigController(t *testing.T) {
	originalEnabled := config.PolicyRoutingConfigSet.Enabled
	originalConfigs := config.PolicyRoutingConfigSet.Configs
	originalGetKernelVersion := GetKernelVersion
	defer func() {
		config.PolicyRoutingConfigSet.Enabled = originalEnabled
		config.PolicyRoutingConfigSet.Configs = originalConfigs
