...
func TestInitPolicyRouting(t *testing.T) {
	...
	originalPolicyRoutingConfigs := PolicyRoutingConfigSet.Configs
	...
	defer func() {
		PolicyRoutingConfigSet.Configs = originalPolicyRoutingConfigs
	...
	originalGetNodeName := nodeinfo.GetNodeName

	defer func() {
		RouteGet = originalRouteGet
		LinkByIndex = originalLinkByIndex
		nodeinfo.GetNodeName = originalGetNodeName
	}()

	RouteGet = func(ip net.IP) ([]netlink.Route, error) {
	...
	nodeinfo.GetNodeName = func() (string, error) {
		return nodeName, nil
	}

	if err := InitPolicyRouting(context.Background(), fakeClient, nodeName); err != nil {
