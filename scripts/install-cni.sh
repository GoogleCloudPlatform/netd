...
if [ -w /host/etc/cni/net.d ]; then
  ...
  node_ipv6_addr=$(curl "http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/ipv6s" -H "Metadata-Flavor: Google" | jq '.[0]')
  echo "DEBUG: ipv6s is ${node_ipv6_addr:-EMPTY}"
